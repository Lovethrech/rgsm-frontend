...
const readerActionLoading = ref(false)
...
const activeReaders = computed(() =>
  readers.value.filter((reader) => reader.is_active)
)

const inactiveReaders = computed(() =>
  readers.value.filter((reader) => !reader.is_active)
)

const hostelReaders = computed(() =>
  readers.value.filter((reader) => reader.hostel_id)
)

const campusReaders = computed(() =>
  readers.value.filter((reader) => !reader.hostel_id)
)

const readerReport = computed(() => {
  return readers.value.map((reader) => {
    const readerEvents = detectionEvents.value.filter(
      (event) => event.reader_id === reader.id
    )

    const readerLogs = accessLogs.value.filter(
      (log) => log.reader_id === reader.id
    )

    const deniedCount = readerLogs.filter(
      (log) => log.decision === 'denied' || log.decision === 'alert'
    ).length

    return {
      ...reader,
      events_count: readerEvents.length,
      access_logs_count: readerLogs.length,
...
const selectGeofenceForEdit = (zone) => {
  selectedGeofence.value = zone

  const rules = zone.rules || {}

  geofenceRuleForm.value = {
    name: zone.name || '',
    description: zone.description || '',
    zone_type: zone.zone_type || '',
    allowed_gender: rules.allowed_gender || '',
    time_start: rules.time_start || '05:00',
    time_end: rules.time_end || '23:00',
    allow_roles: rules.allow_roles || rules.allowed_roles || []
  }
}

const toggleAllowedRole = (role) => {
  const roles = geofenceRuleForm.value.allow_roles

  if (roles.includes(role)) {
    geofenceRuleForm.value.allow_roles = roles.filter((item) => item !== role)
  } else {
    geofenceRuleForm.value.allow_roles = [...roles, role]
  }
}

const saveGeofenceRules = async () => {
  if (!selectedGeofence.value) {
    authError.value = 'Select a geofence zone first.'
    return
  }

  geofenceActionLoading.value = true
  authError.value = ''

  try {
    const updatedRules = {
      allow_roles: geofenceRuleForm.value.allow_roles,
      time_start: geofenceRuleForm.value.time_start,
      time_end: geofenceRuleForm.value.time_end
    }

    if (geofenceRuleForm.value.allowed_gender) {
      updatedRules.allowed_gender = geofenceRuleForm.value.allowed_gender
    }

    const { data, error } = await supabase
      .from('geofence_zones')
      .update({
        name: geofenceRuleForm.value.name,
        description: geofenceRuleForm.value.description,
        zone_type: geofenceRuleForm.value.zone_type,
        rules: updatedRules,
        updated_at: new Date().toISOString()
      })
      .eq('id', selectedGeofence.value.id)
      .select()
      .single()
    ...
    geofences.value = geofences.value.map((zone) =>
      zone.id === data.id ? data : zone
    )

    selectedGeofence.value = data
  } catch (error) {
    console.error('Failed to update geofence rules:', error)
    authError.value = error?.message || 'Failed to update geofence rules.'
  } finally {
    geofenceActionLoading.value = false
  }
}

const clearSelectedGeofence = () => {
      ...
      <section class="reader-detail-panel">
        ...
        <div v-else class="placeholder-block">
          ...
          <p>Choose a reader from the list to view device details and control its status.</p>
            ...
            <template v-else-if="activeSection === 'geofences'">
              <article class="panel full-panel">
                <div class="panel-header">
                  <div>
                    <h2>Geofence Zone Rules</h2>
                    <p>View and manage rule-based RFID access control for campus geofence zones.</p>
                  </div>

                  <span class="panel-badge">Rule Engine</span>
                </div>

                <div class="geofence-rules-layout">
                  <section class="geofence-zone-list">
                    <h3>Campus Geofence Zones</h3>

                    <div class="geofence-rule-cards">
                      <button
                        v-for="zone in geofences"
                        :key="zone.id"
                        type="button"
                        class="geofence-rule-card"
                        :class="{ 'selected-geofence-card': selectedGeofence?.id === zone.id }"
                        @click="selectGeofenceForEdit(zone)"
                      >
                        <div>
                          <h4>{{ zone.name }}</h4>
                          <p>{{ zone.description || 'No description provided.' }}</p>
                        ...
                        <div class="geofence-rule-meta">
                          <span>{{ zone.zone_type || 'zone' }}</span>
                          <span v-if="zone.rules?.allowed_gender">
                            {{ zone.rules.allowed_gender }} only
                          </span>
                          <span>
                            {{ zone.rules?.time_start || 'N/A' }} - {{ zone.rules?.time_end || 'N/A' }}
                          </span>
                        </div>
                      </button>

                      <p v-if="!geofences.length" class="empty-text">
                        No geofence zones found.
                      </p>
                    </div>
                  </section>

                  <section class="geofence-editor-panel">
                    <div v-if="selectedGeofence">
                      <div class="editor-header">
                        <div>
                          <h3>Edit Rules</h3>
                          <p>{{ selectedGeofence.name }}</p>
                        </div>

                        <button type="button" class="warning-btn small-action" @click="clearSelectedGeofence">
                          Clear
                        </button>
                      </div>
                      ...
                      <div class="settings-form">
                        ...
                        <label>
                          Description
                          <input
                            v-model="geofenceRuleForm.description"
                            type="text"
                            :disabled="profile?.role !== 'global_admin'"
                          />
                        </label>
                        ...
                        <label>
                          Zone Type
                          <input
                            v-model="geofenceRuleForm.zone_type"
                            type="text"
                            :disabled="profile?.role !== 'global_admin'"
                          />
                        </label>
                        ...
                        <label>
                          Gender Restriction
                          <select
                            v-model="geofenceRuleForm.allowed_gender"
                            :disabled="profile?.role !== 'global_admin'"
                          >
                        ...
                        <div class="time-rule-grid">
                          <label>
                            Access Start Time
                            <input
                              v-model="geofenceRuleForm.time_start"
                              type="time"
                              :disabled="profile?.role !== 'global_admin'"
                            />
                          </label>
                          ...
                          <label>
                            Access End Time
                            <input
                              v-model="geofenceRuleForm.time_end"
                              type="time"
                              :disabled="profile?.role !== 'global_admin'"
                            />
                          </label>
                        ...
                        <div class="role-selector">
                          ...
                          <div class="role-chip-grid">
                            <button
                              v-for="role in availableAccessRoles"
                              :key="role"
                              type="button"
                              class="role-chip"
                              :class="{ 'role-chip-active': geofenceRuleForm.allow_roles.includes(role) }"
                              :disabled="profile?.role !== 'global_admin'"
                              @click="toggleAllowedRole(role)"
                            >
                              {{ role.replaceAll('_', ' ') }}
                            </button>
                          </div>
                        ...
                        <button
                          type="button"
                          class="safe-btn"
                          :disabled="geofenceActionLoading || profile?.role !== 'global_admin'"
                          @click="saveGeofenceRules"
                        >
                          {{ geofenceActionLoading ? 'Saving...' : 'Save Geofence Rules' }}
                        </button>

                        <p v-if="profile?.role !== 'global_admin'" class="empty-text">
                          Only Global Admin users can edit geofence rules.
                        </p>
                      </div>
                    </div>

                    <div v-else class="placeholder-block">
                      <h3>Select a geofence zone</h3>
                      <p>Choose a zone from the left to view or edit its access control rules.</p>
                    </div>
                  </section>
                </div>
              </article>
...
.geofence-zone-list h3,
.geofence-editor-panel h3 {
  margin-top: 0;
  color: #dbeafe;
}
...
.geofence-rule-card:hover,
.selected-geofence-card {
  border-color: rgba(96, 165, 250, 0.45);
  background: rgba(59, 130, 246, 0.13);
}

.geofence-rule-card h4 {
  margin: 0;
  color: #ffffff;
}

.geofence-rule-card p {
  margin: 0.35rem 0 0;
  color: #94a3b8;
  font-size: 0.85rem;
}
